...
class RandomBridges:
    ...
    def sampling_dfs(self, vertex):
        ...
        for adjacent in self.adj_list[vertex]:
            if self.colors[adjacent] == "white":
                ...
                rand = np.uint64(np.random.randint(0, np.iinfo(np.uint64).max, dtype=np.uint64))
    ...
    def find_2bridges(self, adj_list, sort_fun):
        ...
        for first_key in self.adj_list:
            for second_key in self.adj_list[first_key]:
                if (second_key, first_key) not in [edge[self.EDGE_NUM] for edge in self.edges]:
        ...
        for i in range(len(sorted_args) - 1):
            cluster_size+=1
            if self.edges[sorted_args[i]][self.SAMPLE_NUM] != self.edges[sorted_args[i+1]][self.SAMPLE_NUM]:
                if (cluster_size > 1):
                    self.bridges.append([])
                    j = i - cluster_size + 1
                    while (j < i):
                        self.bridges[current_cluster].append((self.edges[sorted_args[j]][self.EDGE_NUM]))
                        j+=1
                    cluster_size = 0
                    current_cluster+=1
        return self.bridges
